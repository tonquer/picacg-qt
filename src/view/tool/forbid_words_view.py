...
from PySide6.QtWidgets import QWidget, QListWidgetItem, QHBoxLayout, QLineEdit, QPushButton, QLabel, QCheckBox

from component.dialog.base_mask_dialog import BaseMaskDialog
from config.setting import Setting
from interface.ui_forbid_words import Ui_ForbidWords
...
class ForbidWordsView(BaseMaskDialog, Ui_ForbidWords, QtTaskBase):
    ...
    def __init__(self, parent, owner, bookId=""):
        ...
        self.categoryBox.setChecked(Setting.IsForbidCategory.value)
        self.tagBox.setChecked(Setting.IsForbidTag.value)
        self.titleBox.setChecked(Setting.IsForbidTitle.value)
    ...
    def Init(self):
        ...
        for name in CateGoryMgr().allCategorise:
            self.AddItem(name, isDef=True)
        for name in Setting.AddForbidWords.value:
            self.AddItem(name, isDef=False)
        return
    ...
    def AddItem(self, name, isDef=False):
        item = QListWidgetItem(self.listWidget)
        widget = ForbidWordsItem(name, item, True, self.listWidget)
        # 绑定删除信号
        widget.isDef = isDef
        ...
        if Converter('zh-hans').convert(name) in Setting.ForbidWords.value:
            item.setCheckState(Qt.Checked)
            item.setSelected(True)
        else:
            item.setCheckState(Qt.Unchecked)
    ...
    def AddEditItem(self, name):
        ...
        item = QListWidgetItem(self.listWidget)
        widget = ForbidWordsItem(name, item, False, self.listWidget)
        # 绑定删除信号
    ...
    def AddAllItem(self):
        item = QListWidgetItem(self.listWidget)
        widget = ForbidWordsItem(Str.GetStr(Str.All), item, True, self.listWidget)
        item.setSizeHint(widget.sizeHint())
        self.listWidget.setItemWidget(item, widget)
        # if widget.fid == self.fid:
        #     item.setSelected(True)
    ...
    def _DoAddItem(self, item):
        ...
        if not name:
            return
        if name in Setting.AddForbidWords.value:
    ...
    def _MoveBookToFold(self):
        ...
        for i in range(self.listWidget.count()):
            item = self.listWidget.item(i)
            w = self.listWidget.itemWidget(item)
            if item.isSelected() and w.lineEdit.text():
                allcategory.append(Converter('zh-hans').convert(w.lineEdit.text()))
        Setting.ForbidWords.SetValue(allcategory)
        Setting.IsForbidTag.SetValue(self.tagBox.isChecked())
        Setting.IsForbidCategory.SetValue(self.categoryBox.isChecked())
        Setting.IsForbidTitle.SetValue(self.titleBox.isChecked())
