...
class TaskConvert(TaskBase, QtTaskBase):
    def __init__(self) -> None:
    ...
    def StartWaifu2x(self, task):
        assert isinstance(task, QConvertTask2)
        path = task.in_path
        if task.pictureIndex >= len(task.pictureList):
            return Str.Ok

        nextFile = task.pictureList[task.pictureIndex]
        nextFile2 = os.path.join(path, nextFile)
        savePath = os.path.join(task.to_path, nextFile)
        TaskWaifu2x().AddConvertTaskByPath(nextFile2, savePath, self.StartWaifu2xBack, task.taskId)
        return Str.Waiting

    def StartWaifu2xBack(self, saveData, status, backParam, tick):
        return
